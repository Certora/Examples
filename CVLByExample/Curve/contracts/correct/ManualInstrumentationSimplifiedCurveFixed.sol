...
contract ManualInstrumentationSimplifiedCurveFixed is ReentrancyGuard{
...
contract scenario {
    ...
    function init_scenario(/*address other_user*/) payable public returns(address) {
        ...
        ManualInstrumentationSimplifiedCurveFixed _curve = new ManualInstrumentationSimplifiedCurveFixed{value: deposited_ether_before}(address(lp_token), address(token));
...
contract attacker{
    ManualInstrumentationSimplifiedCurveFixed public attacked;
    ERC20 token;
    CurveTokenExample lp_token;

    constructor(address attackedAddr){
        attacked = ManualInstrumentationSimplifiedCurveFixed(attackedAddr);
