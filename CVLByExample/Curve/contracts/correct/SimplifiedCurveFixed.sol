...
contract SimplifiedCurveFixed is ReentrancyGuard{
...
contract scenario {
    ...
    function init_scenario(/*address other_user*/) payable public returns(address) {
        ...
        SimplifiedCurveFixed _curve = new SimplifiedCurveFixed{value: deposited_ether_before}(address(lp_token), address(token));
...
contract attacker{
    SimplifiedCurveFixed public attacked;
    ERC20 token;
    CurveTokenExample lp_token;

    constructor(address attackedAddr){
        attacked = SimplifiedCurveFixed(attackedAddr);
