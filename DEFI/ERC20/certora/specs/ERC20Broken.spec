...
rule transferSpec() {
    ...
    assert balance_sender_after == balance_sender_before - amount, "transfer must decrease sender's balance by amount";
    assert balance_recip_after == balance_recip_before + amount, "transfer must increase recipient's balance by amount";
}
...
rule transferReverts() {
    ...
    assert lastReverted, "transfer(recip,amount) must revert if sender's balance is less than `amount`";
}
...
rule onlyApproveIncreasesAllowance() {
    ...
    f(e, args);
    mathint allowance_after = allowance(holder, spender);
    satisfy allowance_after > allowance_before => (f.selector == sig:approve(address, uint).selector), "only approve and increaseAllowance can increase allowances";
}
