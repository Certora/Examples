error dummy file:240:72: Illegal character: \
/***
...
definition poll_functions(method f) returns bool = f.selector == sig:withdraw(uint256).selector || f.selector == sig:deposit(uint256).selector || f.selector == sig:flashLoan(address, uint256).selector;
...
rule splitDepositFavoursTheContract(env e) {
    ...
    uint256 sharesA = deposit(e, amountA);
...
rule splitWithdrawFavoursTheContract(env e) {
    ...
    uint256 amountA = withdraw(e, sharesA);
