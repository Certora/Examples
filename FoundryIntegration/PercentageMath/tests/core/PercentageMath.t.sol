...
contract PercentageMathTests is Test {
  ...
  function test_percentMul_fuzz_no_expectRevert(uint256 value, uint256 percentage) public {
    if (
      (percentage == 0 ||
        (value > (type(uint256).max - w.HALF_PERCENTAGE_FACTOR()) / percentage) == false) == false
    ) {
  ...
  function test_percentMul_fuzz_wrong_assert(uint256 value, uint256 percentage) public {
    if (
      (percentage == 0 ||
        (value > (type(uint256).max - w.HALF_PERCENTAGE_FACTOR()) / percentage) == false) == false
    ) {
  ...
  function test_percentDiv_fuzz_no_expectRevert(uint256 value, uint256 percentage) public {
    if (percentage == 0 || value > (type(uint256).max - (percentage / 2)) / w.PERCENTAGE_FACTOR()) {
      w.percentDiv(value, percentage);
    } else {
      assertEq(
  ...
  function test_percentDiv_fuzz_wrong_assert(uint256 value, uint256 percentage) public {
    if (percentage == 0 || value > (type(uint256).max - (percentage / 2)) / w.PERCENTAGE_FACTOR()) {
      vm.expectRevert();
      w.percentDiv(value, percentage);
    } else {
  ...
  function test_percentMul() public {
  ...
  function test_percentDiv() public {
